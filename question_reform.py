...
def pre_proc(sent):
  ...
  print(f'sent after pre_poc: {norm}')
  return norm
...
def test_input():
  ...
  while True:
    ...
    if user[-1] not in brk:
      user += '.'
    user = pre_proc(user)
    temp = hist +' '+ user if hist != "" else user
    if hist == "":
      print('empt str')
    else:
      print(hist) 
...
### NOTE ###
# bộ en_core ko ổn vs TV -> thử dịch sang TA, hoặc tệ hơn là chấp nhận lấy entity cũ bị nhiễu
# nguyên nhân: là bộ xử lí TA nên ko hoàn toàn phù hợp vs TV
# bộ entity đôi còn chưa hoàn toàn chính xác(hình ảnh)
...
### TODO ###
# làm cách nào để thêm đại từ vào các câu bị rút gọn đại từ (vẫn chưa biết cách làm do vẫn chưa biết dùng pp nào để xác định 
# vị trí thêm từ và thêm từ gì hoặc entity gì)
# làm cách nào để bắt được các từ khoá trsearchong câu câu(có thể thử tf-idf) -> có thể ko cần do đã có bộ ner và pre_proc để
# chuyển câu hỏi sang statement để search
